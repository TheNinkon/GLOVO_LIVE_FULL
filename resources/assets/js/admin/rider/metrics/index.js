...
$(document).ready(function () {
  ...
  function getFilterParams() {
    ...
    const costoPorHora = parseFloat($('#costo_por_hora').val()) || 15.0;
    const gananciaPorPedido = parseFloat($('#ganancia_por_pedido').val()) || 5.5;
    ...
    return {
      ...
      ganancia_por_pedido: gananciaPorPedido
    };
  }
  ...
  function fetchKpis(params = {}) {
    const { costo_por_hora, ganancia_por_pedido } = params;
    $.ajax({
      url: kpisApiUrl,
      data: params,
      success: function (data) {
        // Recalcular los valores de los KPIs con los nuevos parámetros
        const totalHours = parseFloat(data.total_hours);
        const totalOrders = parseInt(data.total_orders);
        const totalGrossProfit = totalOrders * ganancia_por_pedido;
        const totalOperationalCost = totalHours * costo_por_hora;
        const totalNetProfit = totalGrossProfit - totalOperationalCost;
        ...
        let kpisHtml = `
                ...
                <div class="d-flex align-items-center mb-2 pb-1">
                  <div class="avatar me-2">
                    <span class="avatar-initial rounded bg-label-primary">
                      <i class="ti tabler-package ti-md"></i>
                    </span>
                  </div>
                ...
                <div class="d-flex align-items-center mb-2 pb-1">
                  <div class="avatar me-2">
                    <span class="avatar-initial rounded bg-label-success">
                      <i class="ti tabler-rocket ti-md"></i>
                    </span>
                  </div>
                  <h4 class="ms-1 mb-0">${data.avg_ratio.toFixed(2)}</h4>
                </div>
                ...
                <div class="d-flex align-items-center mb-2 pb-1">
                  <div class="avatar me-2">
                    <span class="avatar-initial rounded bg-label-info">
                      <i class="ti tabler-hourglass-low ti-md"></i>
                    </span>
                  </div>
            ...
            <div class="card">
              <div class="card-body d-flex justify-content-between align-items-center">
                <div class="card-title mb-0">
                  <h5 class="mb-0 me-2">${formatMoney(totalGrossProfit, '€')}</h5>
                  <small>Ganancia Bruta</small>
                </div>
                <div class="card-icon">
                  <span class="badge bg-label-success rounded-circle p-2">
                    <i class="ti tabler-arrow-up-right ti-sm"></i>
                  </span>
                </div>
              </div>
            </div>
          ...
          <div class="col-lg-3 col-sm-6 mt-4">
            <div class="card">
              <div class="card-body d-flex justify-content-between align-items-center">
                <div class="card-title mb-0">
                  <h5 class="mb-0 me-2">${formatMoney(totalOperationalCost, '€', true)}</h5>
                  <small>Costo Operativo</small>
                </div>
                <div class="card-icon">
                  <span class="badge bg-label-danger rounded-circle p-2">
                    <i class="ti tabler-arrow-down-right ti-sm"></i>
                  </span>
                </div>
              </div>
            </div>
          ...
          <div class="col-lg-3 col-sm-6 mt-4">
            <div class="card">
              <div class="card-body d-flex justify-content-between align-items-center">
                <div class="card-title mb-0">
                  <h5 class="mb-0 me-2">${formatMoney(totalNetProfit, '€')}</h5>
                  <small>Ganancia Neta</small>
                </div>
                <div class="card-icon">
                  <span class="badge bg-label-info rounded-circle p-2">
                    <i class="ti tabler-currency-euro ti-sm"></i>
                  </span>
                </div>
              </div>
            </div>
      ...
      success: function (data) {
        ...
        if (data.data.length > 0) {
          data.data.forEach(metric => {
            ...
            const grossProfit = metric.pedidos_entregados * ganancia_por_pedido;
            const operationalCost = metric.horas * costo_por_hora;
            const netProfit = grossProfit - operationalCost;

            const netClass = netProfit >= 0 ? 'text-success' : 'text-danger';
            const netAmount =
              netProfit >= 0 ? `${formatMoney(netProfit, '€')}` : `${formatMoney(netProfit, '€', true)}`;

            let vehicleIcon = '';
            if (metric.transport === 'BICYCLE') {
              vehicleIcon = '<i class="ti tabler-bike"></i>';
            } else if (metric.transport === 'MOTORBIKE') {
              vehicleIcon = '<i class="ti tabler-motorbike"></i>';
            }
            ...
            tableRows += `
              <tr>
                ...
                <td>${formatMoney(operationalCost, '€', true)}</td>
